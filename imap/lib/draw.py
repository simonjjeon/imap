...
def draw_line(line, color=None, reference_line=False, label="", is_route=False):
    x = [point.x for point in line]
    y = [point.y for point in line]

    if reference_line:
        color = "r" if is_route else "k"
        alpha = 0.8 if is_route else 0.2
        ax.plot(x, y, color, linestyle="dashed", linewidth=10, alpha=alpha, label=label)
    else:
        if color:
