...
from imap.lib.proto_utils import write_pb_to_text_file, write_pb_to_bin_file
...
class Opendrive2Apollo(Convertor):
    ...
    def convert_roads(self, route=[]):
        for _, xodr_road in self.xodr_map.roads.items():
            ...
            road_id = str(pb_road.id.id)
            draw_line(
                xodr_road.reference_line,
                "r",
                reference_line=True,
                label="Road " + road_id,
                is_route=any([road_id == road.road_id for road in route]),
            )
